#K.Palof ADF&G 
# updated 12-6-16
# Just function for bootstrap for tanner crab.  
# This function is modified slightly to speed up the bootstrap process
...
crabCSAB <- function (area = NULL, M = NULL, initial = c(NA, NA, NA, NA, NA), uprn = NA, graph = TRUE)
{  
  year = area$Year
  catch = area$Catch..Number.
  preR = area$PreR
  recr = area$Recruit
  post = area$PostR
  csT= area$Catch..Survey.Tau 
  sTs =area$Survey.Tau
  LegWt=area$Legal.Weight
  PreRWt=area$Prerecruit.Weight 
  w = area$w
  outs <- NULL
