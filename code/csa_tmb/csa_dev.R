...
datfile <- paste0(here::here(), "/data/rkc/", survey.location, "/", survey.location, "_model_input_", cur_yr, ".rds")

# extract data objects, compute taus, combine in list
{dat <- as.matrix(readRDS(datfile))
  colnames(dat) <- NULL
  ...
  #end = 2+length(yrs)
  #catch_num <- as.numeric(dat[4:end, 2])
  catch_num <- unname(unlist(dat[[4]][2]))
  # catch mid date
  #catch_md <-  as.numeric(dat[4:end, 3])
  catch_md <- unname(unlist(dat[[4]][3]))
  # obs index
  #start = end+1
  #end = start+length(yrs)-1
  #obs_index <- dat[start:end, 2:(nstage+1)]
  obs_index <- unname(unlist(dat[[5]][2:(nstage + 1)]))
  # index mid date
  #index_md <- dat[start:end, 5]
  index_md <- unname(unlist(dat[[5]][nstage + 2]))
  # wt survey
  #start = end+1
  #end = start+length(yrs)-1
  #wt_survey <- dat[start:end, 1:3]
