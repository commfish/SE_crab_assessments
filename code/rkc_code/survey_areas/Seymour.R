...
histdat <- read.csv(paste0('./results/rkc/', survey.location, 
                           '/', pr_yr, '/SC_perpot_all_', pr_yr, '.csv'))
           ## !!!!  this file will be 'SC_perpot_all_pr_yr' and just get updated with current years data.
...
# should have removed experimental ones here. 
dat1 %>%
...
dat_soak <- dat1 %>%
  ...
  mutate(time_hauled = as.POSIXct(Time.Hauled,format="%Y-%m-%d-%Y %H:%M:%S",tz=Sys.timezone())) %>%
...
dat5 %>% 
  select(Year, Location, Trip.No, Pot.No, Density.Strata.Code, Missing, Juvenile, Large.Females, 
         Post_Recruit, Pre_Recruit, Recruit, Small.Females, Area, npots, inverse_n, weighting) %>% 
  rename(Strata.Code = Density.Strata.Code) -> dat6
...
long_t(dat5_cur_yr, baseline, cur_yr, 'Seymour', 'Seymour Canal')
...
weights(dat1, 2.87, 6.438, "Seymour", cur_yr)
...
poor_clutch_long(poorclutch_current, 'Seymour', cur_yr)
...
egg_percent(largef_all, 'Seymour', cur_yr)
# output saved as egg_percent_mean.csv

### total stock health table -----------------------
total_health('Seymour', cur_yr)
