...
##
## This script handles Peril-specific data wrangling, then calls the
...
# ============================================================================
area_name <- "PS"          # short label for filenames and plot titles
cur_yr    <- 2024           # current analysis year

# ============================================================================
# DATA WRANGLING (area-specific -- will be streamlined in the future)
# ============================================================================

df <- read.csv("CSA_excel/Peril Straits 2023 input data RTMB.csv")

this_year_cpue <- read.csv("results/rkc/Peril/2024/PS_CPUE_2024.csv") %>% 
  filter(Year == cur_yr)
this_year_crab_weights <- read.csv("results/rkc/Peril/2024/maleweights.csv") %>%
  filter(Year == cur_yr)


df$Catch..Number.[45] <- 0       # No commercial catch in Gambier in 24/25(fishery closed)
df$Catch.Mid.Date[45] <- NA 
...
new_line <- data.frame(
  ...
  Survey.Mid.Date    = "22-Jul-24", #approximated for 2024 based on 2022
...
######################################################################################################################################################
# TAUs- AGR FLAG - fix here and pybus- AGR HERE!!
N2 <- 0.8435  # fallback value from Excel $N$2- of Peril specifically, from Excel

CATCH_SURVEY_TAU    <- rep(0, length(CATCH_MIDDATE))
CATCH_SURVEY_TAU[1] <- 0 #specific for areas- peril
for (i in 2:length(CATCH_MIDDATE)) {
  if (CATCH[i - 1] == 0) {
    CATCH_SURVEY_TAU[i] <- N2 #if the catch is 0, the catch_survey_tau=
  } else {
...
CATCH_SURVEY_TAU[is.na(CATCH_SURVEY_TAU)] <- N2 #will likely have to expand this to other area. Make sure this is appropriate here- a band aid but it works
...
# ============================================================================
#FLAG - excel estimated the big number and they are scaled as a separate step. Not sure why, but thay could be why my q's are different?
T12_start <- 36.04340865 / 100   # from 2025 Peril CSA (Q2)  #oh, I should maybe use the bigger value to estimate and scale it for the output
q_start   <- 323.088895 / 1e6   # from 2025 Gambier Excel CSA (Q3)
...
boot <- bootstrap_ci(mod$pop_mod, data, pars, map,
                     ...
                     seed = 42, verbose = TRUE, newtonsteps = 0) #does not like newtonsteps for peril
...
# ============================================================================
df_excel <- read.csv("CSA_excel/Peril Straits 2024 Excel results comparison.csv")
...
#SAVE output csv
write.csv(output_df, paste0("results/rkc/Peril/", cur_yr, "/CSA_output_2", cur_yr, ".csv"), row.names = FALSE) #fkag- output date is bad
