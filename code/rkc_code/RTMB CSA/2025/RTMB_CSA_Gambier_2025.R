...
##
## This script handles Gambier-specific data wrangling, then calls the
...
# ============================================================================
area_name <- "GB"          # short label for filenames and plot titles
...
df <- read.csv("CSA_excel/Gambier Bay 2025 input data RTMB.csv")

this_year_cpue <- read.csv("results/rkc/Gambier/2025/GB_CPUE_2025.csv") %>% #results from the CPUE standardization and summary script
  filter(Year == cur_yr)
this_year_crab_weights <- read.csv("results/rkc/Gambier/2025/maleweights.csv") %>% #from that same R summary
  filter(Year == cur_yr)

df$Catch..Number.[47] <- 0       # No commercial catch in Gambier in 24/25(fishery closed)
df$Catch.Mid.Date[47] <- NA 
...
new_line <- data.frame(
  ...
  Survey.Mid.Date    = "8-Aug-25", #automate date entry later
...
N2 <- 0.7578  # fallback value from Excel $N$2- of Gambier specifically, from Excel

CATCH_SURVEY_TAU    <- rep(0, length(CATCH_MIDDATE))
CATCH_SURVEY_TAU[1] <- 0 #specific for areas-
...
# ============================================================================
T12_start <- 74.7654358311386 / 100   # from 2025 Gambier Excel CSA (Q2) 
q_start   <- 514.265221587 / 1e6   # from 2025 Gambier Excel CSA (Q3)
...
# ============================================================================
df_excel <- read.csv("CSA_excel/Gambier Bay 2025 Excel results comparison.csv")
...
#SAVE output csv
write.csv(output_df, paste0("results/rkc/Gambier/", cur_yr, "/CSA_output_2", cur_yr, ".csv"), row.names = FALSE) #fkag- output date is bad
