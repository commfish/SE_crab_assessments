### Pybus Bay RTMB CSA 2025 ###
## Alex Reich
## 4/22/26

#pybus supringly runs well
...
# ============================================================================
area_name <- "PB"          # short label for filenames and plot titles
...
df <- read.csv("CSA_excel/Pybus Bay 2025 input raw data RTMB.csv")

this_year_cpue <- read.csv("results/rkc/Pybus/2025/PB_CPUE_2025.csv") %>% #results from the CPUE standardization and summary script
  filter(Year == cur_yr)
this_year_crab_weights <- read.csv("results/rkc/Pybus/2025/maleweights.csv") %>% #from that same R summary
  filter(Year == cur_yr)

df$Catch..Number.[47] <- 0       
df$Catch.Mid.Date[47] <- NA #says "No Catch", both in Pybus and Seymour. Do I need to adjust all NO CATCH instaces to say NA? AGR flag
...
new_line <- data.frame(
  ...
  Survey.Mid.Date    = "7-Aug-25", #automate date entry later- UNIQUE FOR EACH AREA
...
# ============================================================================
T12_start <- 75.8430444190728 / 100   # from 2025 Pybus CSA (Q2) 
q_start   <- 179.360430632595 / 1e6   # from 2025 Pybus CSA (Q3)
...
data <- list(
  ...
  CATCH_SURVEY_TAU = CATCH_SURVEY_TAU #NA's in here, are we ok with that?? AGR FLAG!!
)

pars <- list(
  ln_mean_rec          = log(1.5), #can try diff mean start values
...
# ============================================================================
df_excel <- read.csv("CSA_excel/Pybus Bay 2025 Excel results comparison.csv")
...
#SAVE output csv
write.csv(output_df, paste0("results/rkc/Pybus/", cur_yr, "/CSA_output_2", cur_yr, ".csv"), row.names = FALSE) #fkag- output date is bad
